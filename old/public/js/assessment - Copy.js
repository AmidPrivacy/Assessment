...
$(".assesment-completed").click(function() {
    ...
    assessmentCount = criteryList.length * 3;
...
$(function() {
                ...
                success: function(data) {
                    ...
                    data.calls.forEach((item, index) => {
                        let allTimer =
                            ...
                            '<div class="supervisor-counter not-listen-time" data-second="0"><label class="minutes">00</label>:<label class="seconds">00</label></div>' +
                            '<div class="supervisor-counter special-calculate-time" data-second="0"><label class="minutes">00</label>:<label class="seconds">00</label></div>';

                        if (item.assessmentTime !== null) {
                            assessmentTimeSum += item.assessmentTime;
                            playTimeSum += item.playTime;
                            notListenTime += item.unPlayTime;
                            specialTimeSum += item.specialTime;
                            ...
                            allTimer =
                                ...
                                secondsToTimestamp(item.playTime)
                                    ...
                                    .split(":")[1] +
                                "</label></div>" +
                                '<div class="supervisor-counter not-listen-time" data-second="' +
                                item.unPlayTime +
                                '"><label class="minutes">' +
                                secondsToTimestamp(item.unPlayTime)
                                    .substring(3)
                                    .split(":")[0] +
                                '</label>:<label class="seconds">' +
                                secondsToTimestamp(item.unPlayTime)
                                    ...
                                    .split(":")[1] +
                                "</div>" +
                                '<div class="supervisor-counter special-calculate-time" data-second="' +
                                item.specialTime +
                                '"><label class="minutes">' +
                                secondsToTimestamp(item.specialTime)
                                    .substring(3)
                                    .split(":")[0] +
                                '</label>:<label class="seconds">' +
                                secondsToTimestamp(item.specialTime)
                        ...
                        $(".step-container-calls tbody").append(
                            "<tr data-start-date='" +
                                ...
                                (item.count ? item.count : criteryCount * 3) +
                                "</td></tr>"
                        );
                    ...
                    $(".all-listening-times .not-listen-time").text(
                        secondsToTimestamp(notListenTime).substring(3)
                    );
                    ...
                    $(".all-listening-times .special-calculate-time").text(
                        secondsToTimestamp(specialTimeSum).substring(3)
                    );
    ...
    $(".action-button").click(function() {
        var beginDate = $(".step-container-date")
            .find(".begin-date")
            .val();
        var endDate = $(".step-container-date")
            .find(".end-date")
            .val();
                ...
                success: function(data) {
                    ...
                    if (data.services.length > 0) {
                        ...
                        if (callCount > 0) {
                            ...
                            $(".selected-not-days span").text(
                                data.work + " (" + data.notWork + ")"
                            );
                        }
                    } else {
    ...
    $(".call-refresh").click(function() {
        ...
        $.ajax({
            type: "get",
            url: `/research-call/${id}`,
            dataType: "json",
            success: function(data) {
                if (data.status == 200) {
                    document.querySelector("#confirm-modal").close();
                    // console.log(data.endDate);
                    $(".active-line").attr("data-start-date", data.startDate);
                    $(".active-line").attr(
                        "data-end-date",
...
$(document).on("click", ".add-critery", function() {
    ...
    parent.find(".selected-items").append(
...
$(document).on("click", ".call-assessment", function() {
    var criterias = $(this)
        .parent()
        .find(".selected-items .list-group-item");
    ...
    let specialTime =
        (Number(
            $(".active-line .supervisor-counter")
                .eq(1)
                .attr("data-second")
        ) -
            wavesurfer.getDuration() -
    ...
    let check = commonAssessment.some(element => element.call == callInfo.call);
    ...
    console.log(allCriteryList);
    $(".assessment-statistics .calls-completed-score").text(
        assessmentCount.toFixed(2) +
            " / " +
            Math.round(
                (assessmentCount.toFixed(2) / (criteryList.length * 3)) * 100
            ) +
            "%"
    );
    ...
    if (
        criteraList.length > 0 ||
        $(this)
            .next()
            .val().length > 0
    ) {
        $(".assessment-call-loading").fadeIn();
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status == 200) {
                    ...
                    $(".active-line .not-listen-time").text(
                        secondsToTimestamp(notPlayTime).substring(3)
                    );

                    $(".active-line .not-listen-time").attr(
                        "data-second",
                        notPlayTime
                    );

                    $(".active-line .special-calculate-time").text(
                        secondsToTimestamp(specialTime).substring(3)
                    );

                    $(".active-line .special-calculate-time").attr(
                        "data-second",
                        specialTime
                    );
                    ...
                    if (callTypes ? callTypes.length : 0 > 0) {
                        $(".callTypes option").each(function() {
                            if (callTypes.some(item => item == $(this).val())) {
                                type += $(this).text() + "; ";
                            }
                        });
                        type =
                    ...
                    tableRow.each(function() {
                        ...
                        notListenTime += Number(
                            $(this)
                                .find(".not-listen-time")
                                .attr("data-second")
                        );

                        specialTimeSum += Number(
                            $(this)
                                .find(".special-calculate-time")
                                .attr("data-second")
                        );

                        if (
                            $(this)
                                .find(".call-assessment-status")
                                .text() != ""
                        ) {
...
$(document).on("click", ".step-container-calls table tbody tr", function() {
    ...
    // document.querySelector(".checked-critery input").checked = false;
    ...
    if (collapse == 0) {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(data) {
                let totalScore = $(".critery-list").find("li").length * 3;
                ...
                if (data.code == 200) {
                    ...
                    $("#special-calculate-time").val(
                        data.assessmentCritery.specialTime == null
                            ? 0
                            : data.assessmentCritery.specialTime
                    );

                    $("#not-listen-time").val(
                        data.assessmentCritery.unplayTime == null
                            ? 0
                            : data.assessmentCritery.unplayTime
                    );
                    ...
                    if (
                        ...
                        //     ".checked-critery input"
                        // ).checked =
                        //     data.assessmentCritery.wrongSelection == 0
                        //         ? false
                        //         : true;
                    ...
                    xmlhttp.open(
                        "GET",
                        `/get-mediasense/${number}/${beginDate}/${endDate}`,
                        true
                    );
                    ...
                    //     "application/json"
                    // );
                    // xmlhttp.setRequestHeader("Accept", "*/*"); // accept all
...
$(".completed-assessment").click(function() {
    ...
    assesmentList.each(function() {
        if ($(this).find(".op-assessment-checked")[0].checked) {
            score += Number(
                $(this)
                    .find(".assessment-completed-score span")
                    .text()
            );
        ...
        success: function(data) {
            ...
            data.calls.forEach((item, index) => {
                ...
                if (item.criterias !== null) {
                    ...
                    callAssessment.querySelector(
                        ".list-group"
                    ).innerHTML = asessmentCriteriaList;
                    assesmentCount += 1;
                    assesmentScore += item.count;
                }
