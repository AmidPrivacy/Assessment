                ...
                <tr>
                  ...
                  <td> 
                    @if(Auth::user() && $item->is_active === 0 && Auth::user()->role == 0 && $item->assestUser == Auth::user()->id)
                      <a href="/complete-assessment/{{ $item->assessmentId }}" onclick="if (! confirm('Yekunlaşdırmaq istədiyinizə əminsinizmi?')) { return false; }">
                        <button class="btn btn-info complete-assessment">
                          Yekunlaşdır
                        </button>
                      </a>
