  ...
  <body style="background-image: url('/img/operator_background.svg'); background-color: #2090fe33">
    ...
    <div class="container operator-assessment">
      ...
      <div class="all-assessment-tab">
        ...
        <a href="/users">
          <button class="btn btn-info" style="background-color: #c82333; cursor: pointer; border: none"> Digər əməkdaşlar </button>
        </a> 
      </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                {{-- @if(!Auth::user() || Auth::user()->role !==0 ) --}}
                <th> Şikayət/K.S </th>
                <th> Əsaslı/Əsassız Sayı </th>
                {{-- @endif --}}
              </tr>
            </thead>
            <tbody>
              @foreach ($data['assessment'] as $item) 
              <tr class="checked-assessment"  data-id='{{ $item->id }}' style='background-color: {{ $item->is_accept == 0 ? "antiquewhite" : "#fff" }} '>
                <td>  
                  <span class="operator-fullname">{{ explode(" ",$item->begin_date)[0].' -- '.explode(" ",$item->end_date)[0] }}</span> 
                </td>
                <td class="assessment-completed-score"> <span>{{ $item->score_count }} / {{ $item->score_percent }}%</span> </td>
                <td>
                  @if(Auth::user() && Auth::user()->role ==0 && $item->is_accept == 1)
                ...
                <td>
                  <a href="{{ (Auth::user() ? '/assest-calls/':'/op-assest-calls/').$item->id }}">
                    <button class="assessment-details" data-id="{{ $item->id }}" style="left: -45px">Ətraflı</button>  
                  </a> 
                </td>
                {{-- @if(!Auth::user() || Auth::user()->role !==0 ) --}}
                <td>
                  <span class="complaint-count">
                    {{ $item->newComplaintSum }}
                  </span> 
                </td>
                {{-- @endif --}}
                <td>
                  <span class="complaint-count">
                    {{ $item->reasonableComplaintSum.' / '.$item->unreasonableComplaintSum }}
                  </span> 
                </td>
              </tr>
              @endforeach  
