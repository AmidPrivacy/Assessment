      ...
      <section class="section section__menu"> 
        <div class="step-container-operators">
          ...
          <div class="assessment-tab">
              <div class="operators-tab active-tab">
                Hamısı ({{ count($data['allComplaints']) }})
              </div>
          ...
          <div class="table-scrool">
            <table>
              ...
              <tbody class="complaint-tab-content">
                @foreach ($data['allComplaints'] as $item) 
                <tr class="show-tab" data-complaint="new-complaints-tab">
                  ...
                  <td> 
                    <span class="assestment-date">{{ $item->endDate }}</span>
                  </td> 
                  <td> 
                    @if(Auth::user() && $item->is_active === 0 && Auth::user()->role == 0)
                      <button  class="button continue-assestment next-process" data-id="{{ $item->assessmentId }}" 
                        data-operator="{{ strtoupper($item->full_name) }}">
                        Davam et
                      </button>
                    @endif
                    @if($item->beginDate !== 'Qiymətləndirmə edilməyib' && $item->is_active === 1)
                  ...
                  <td>  
                    @if($item->complaint > 0)
                      <span class="complaint-count"> {{ $item->complaint  }} </span> 
                ...
                @foreach ($data['finishedComplaints'] as $item) 
                <tr data-complaint="finished-complaints-tab">
                  <td> 
                    <i class="fas fa-user-circle"></i>
                    <span class="operator-fullname">{{ $item->full_name }}</span> 
                  </td>
                ...
                @foreach ($data['unFinishedComplaints'] as $item) 
                <tr data-complaint="unfinished-complaints-tab">
                  <td> 
                    <i class="fas fa-user-circle"></i>
                    <span class="operator-fullname">{{ $item->full_name }}</span> 
                  </td>
        ...
        <div class="step-container-date">
          ...
          <div class="input-group input-daterange">
              <div class="date-selected">
                ...
                <input type="text" class="form-control begin-date" placeholder="Başlama tarixi seçin" data-date-format="dd-mm-yyyy" readonly>
              </div>
              <div class="date-selected">
                <label>Bitmə tarixi</label>
                <input type="text" class="form-control end-date"  placeholder="Bitmə tarixi seçin" data-date-format="dd-mm-yyyy" readonly>
              </div>
          </div>
        ...
        <div class="step-container-services"> 
          ...
          <div class="statistic-counts">
            ...
            <div class="selected-not-days">
              <b>Gəlmədiyi gün sayı:</b> 
              <span>  </span> 
            </div>
          </div> 
          <div class="clearfix"></div>
          <input type="text" name="gplus" id="checked-count" placeholder="Say daxil edin"/>  
        ...
        <div class="step-container-calls">
          ...
          <div class="col-md-12 audio-player">  
            ...
            <input type="hidden" id="not-listen-time" value="0">
          ...
          <div class="assessment-statistics">
            ...
            <button type="button" class="btn btn-primary">
              Orta bal <span class="badge badge-light calls-completed-score">30</span>
            </button>
          </div>
          <div class="table-scrool">
            <table>
